  ...
  try {
    ...
    const messages = await Promise.all(
      rows.map(async (row) => {
        ...
        const recallSource =
          row.metadata.recallSource === "walrus" ||
          row.metadata.recallSource === "postgres_fallback"
            ? (row.metadata.recallSource as RecallSource)
            : citedMemories.some((m) => m.recallSource === "walrus")
              ? "walrus"
              : undefined;
